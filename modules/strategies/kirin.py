...
def analyze_kirin_phase(klines: list[dict]) -> dict[str, Any]:
    ...
    avg_vol < sum(klines[i]["vol"] for i in range(-60, -30)) / 30 if len(klines) >= 60 else False
    ...
    if (
        trend == "UP"
        and avg_vol > (sum(klines[i]["vol"] for i in range(-60, -30)) / 30 if len(klines) >= 60 else avg_vol) * 1.3
        and up_days >= 18
    ):
        phase = "拉升"
        confidence = 0.8
    ...
    if (
        trend == "DOWN"
        and avg_vol < (sum(klines[i]["vol"] for i in range(-60, -30)) / 30 if len(klines) >= 60 else avg_vol) * 0.8
        and up_days < 10
    ):
        phase = "回落"
        confidence = 0.7
