...
try:
    ...
    # 已废弃：仅在直接运行 `python core/indicators/data_layer.py` 时生效
    # 安装包后（pip install -e .）统一走相对导入，此分支不再需要
    raise ImportError(
        "请使用 'pip install -e .' 安装包后通过 'zt' 命令调用，或通过 'python -m core.indicators.data_layer' 运行"
    )
...
def _build_save_tuple(result: IndicatorResult, today) -> tuple:
    ...
    字段顺序必须与 _SAVE_SQL 列顺序严格一致。
    新增 IndicatorResult 字段时，需同步更新：
    1) _FLOAT_FIELDS / _BOOL_FIELDS / _INT_FIELDS / _STR_FIELDS（控制 load/save）
    2) 本函数的 tuple（需插入到对应位置）
    3) _SAVE_SQL 的列列表
    """
...
# NOTE: 新增 IndicatorResult 字段时，同时更新 _build_save_tuple 和本 SQL 的列列表。
...
def _get_realtime_data(ts_code: str) -> DailyData | None:
    """
    [DEPRECATED] 获取实时/最新行情数据 — 未实现，始终返回 None。
    保留函数体以兼容可能的内部调用，但已从公开 API 移除。
    """
    pass
...
def _step_nana(klines, result):
    result.is_nana = detect_nana_peak_structure(klines)["is_nana"]
...
def _step_sb1(klines, result):
    result.is_sb1 = detect_sb1_break_recover(klines)["is_sb1"]
