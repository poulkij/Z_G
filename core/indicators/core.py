...
from core.database import DB_PATH as _db_path, get_db_connection
...
class DailyData:
    ...
    @classmethod
    def from_dict(cls, klines: list[dict]) -> list["DailyData"]:
        """将 dict 格式 K 线列表转为 DailyData 列表"""
        result = []
...
@dataclass
class IndicatorResult(StockProfile):
    """指标计算结果（继承 StockProfile 的公共字段，追加专属指标）"""

    # KDJ — inherited from StockProfile
    # MACD — inherited from StockProfile
    # RSI — rsi6 inherited from StockProfile
    # MA — ma5/ma20 inherited from StockProfile
    # Bollinger — boll_mid inherited from StockProfile
    # Volume — vol_ratio inherited from StockProfile
    ...
    # 布林带 — boll_mid inherited from StockProfile
    ...
    # 量比 — vol_ratio inherited from StockProfile
